...
.recover_run:
    ...
    ; 这里临时保存到栈，最后跳转前再恢复
    ; 一是为了尽量晚恢复中断标志位，
    ; 二是以防中间有指令改变了eflags的值（实际上mov、pop、push都不影响标志位的值）
    ; -------------------------------------------------------------------------
    mov ecx, [eax + 9 * 4]
    push ecx
    ...
    ; 完全恢复之前的环境（所有寄存器）
    ; -------------------------------------------------------------------------
    ; 因为要恢复任务，继续执行原程序，所以连eax在内都要恢复，
    ; 比如原任务正执行到带返回值的程序的最后一行指令，如果eax没恢复，那任务就出错了，
    ; -------------------------------------------------------------------------
    ; 包括eflags，比如zf位暂存了判断结果，
    ; 原任务一定开了中断了，所以最后一步恢复eflags后，即可立即跳转恢复原流程
    ; -------------------------------------------------------------------------
    mov ecx, [eax + 11 * 4]
    mov eax, [eax + 10 * 4]
    popfd ; eflags（从栈中恢复）
