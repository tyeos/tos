...
interrupt_handler_%1:
    ...
    ; ---------------------------------------------------------------------------------------
    ; 部分中断还会产生错误码，如果有错误码的情况下，还会在eip之后 push 一个 error code
    ; 注意, err code 并不会在 iret 指令中弹出，目的就是为了强制让人处理，
    ; 错误必须处理完然后手动恢复栈，否则使用 iret 指令将返回到错误的地方，即程序出现不可知的问题。
    ; ---------------------------------------------------------------------------------------
    ; 所以进入中断后，初始栈结构为：
    ; ---------------------------------------------------------------------------------------
    ;     (err_code)   ↑ 低地址（ <-esp 当前栈顶）
    ;     eip          ↑
    ;     cs           ↑ （栈的增长方向）
    ;     eflags       ↑
    ...
    add esp, 4  ; 该指令不影响eax的值
    cmp al, 0   ; 判断interrupt_handler_callback的返回值
    popad       ; 恢复现场，将栈中数据弹出，依次传给edi、esi、ebp、esp、ebx、edx、ecx、eax，即ESP会增加32（注：esp原始值会被忽略）

    je .no_error_code ; 根据al的结果，pop指令不影响eflags标志位结果，如果为0不用管，否则要跳过error_code把栈平到eip处
    add esp, 4
