...
interrupt_handler_%1:
    ; ---------------------------------------------------------------------------------------
    ; 在原程序进入中断之前，cpu会先向栈中压入3个(未发生栈切换时)或5个(发生栈切换时)值，再进入中断流程：
    ; 发生栈切换时，执行顺序为：
    ;       push ss
    ;       push esp
    ;       push eflags
    ;       push cs
    ;       push eip
    ; 未发生栈切换时（即使用同一个特权级栈），执行顺序为：
    ;       push eflags
    ;       push cs
    ;       push eip
    ; ---------------------------------------------------------------------------------------
    ; 就相当于在中断前执行的程序中，加入了以上3行或5行push指令，再使用jmp指令无条件跳转，就跳到了这里，
    ; 后面通过iret指令，即可按压入栈按逆顺序弹出，恢复到原程序执行
    ; ---------------------------------------------------------------------------------------
    ; 所以进入中断后，初始栈结构为：
    ; ---------------------------------------------------------------------------------------
    ;     eip          ↑ 低地址（ <-esp 当前栈顶）
